...
- 五部或更多手機／平板可掃描同一 QR Code；沒有裝置編號、房間或同步。
- 每部裝置的播放內容、音量、鏡頭及累計次數只保存在該裝置。
- 相機影像只在瀏覽器內處理，不會上傳、錄製或保存。
- 可選擇人臉、左右揮手或卡片感應；人臉／卡片離開鏡頭 0.5 秒後可再次觸發。
- 卡片模式辨識近似信用卡比例的長方形，穩定出現 0.5 秒後播放合成雙音拍卡聲；不辨識卡面、品牌或文字。
- 四段粵語使用網站內置音檔，不依賴系統語音；輕快音樂使用指定附件第 22 秒起的 30 秒片段。
- 支援 iPhone／Android 手機及 iPad；建議使用最新版 Safari 或 Chrome。
...
然後開啟 `http://127.0.0.1:5200/`。本機預覽只供 Mac 測試；正式手機／平板相機需使用上面的 HTTPS Pages 網址。
...
1. 用 Safari 或 Chrome 開啟固定網址，允許相機。
2. 按「分享 QR」讓其餘裝置掃描同一 QR Code。
3. 每部裝置各自選擇內容，再按「立即啟動」。
